...
--specifically drop this view (remove this to make this a script generic!)

if  EXISTS( select * FROM sys.views where name = 'vwItemSummaryTmp')
	drop view [manheim].[vwItemSummaryTmp]
...
create procedure [dbo].[DeleteRole] @RoleName sysname
as begin 
...
BEGIN
	...
	BEGIN  
		...
		SELECT @sqlCommand =  t.ObjectName FROM @t t WHERE RowID = @i;
		exec [dbo].[DeleteRole] @sqlCommand
